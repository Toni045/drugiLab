...
static long control_ioctl (struct file *filp, unsigned int request, unsigned long arg)
{
	...
	if (_IOC_TYPE(request) != SHOFER_IOCTL_TYPE || _IOC_NR(request) != SHOFER_IOCTL_NR) {
		klog(KERN_WARNING, "Invalid ioctl type or number");
		return -EINVAL;
	}

	if (_IOC_SIZE(request) != sizeof(struct shofer_ioctl)) {
		klog(KERN_WARNING, "Invalid ioctl argument size");
		return -EINVAL;
	}
	...
	to_copy = min(kfifo_len(fifo_in), cmd.count);
	to_copy = min(to_copy, kfifo_avail(fifo_out));

	buffer = kmalloc(to_copy, GFP_KERNEL);
	if (!buffer) {
		retval = -ENOMEM;
		goto out_unlock;
	}

	copied = kfifo_out(fifo_in, buffer, to_copy);
	if (copied > 0)
		copied = kfifo_in(fifo_out, buffer, copied);
